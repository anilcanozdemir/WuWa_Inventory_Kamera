"""Always-on-top Weapon ROI recorder (Start button).

Open Resonators → Weapon tab with Daybreaker visible, then:
  tools\\START_RECORD_WEAPON.bat

Click top-left then bottom-right for: name, level, rank.
Saves debug_out/weapon_rois_*/rois.json for gameROI.
"""
...
STEPS = [
    ("weaponName", "NAME top-left"),
    ("weaponName", "NAME bottom-right"),
    ("weaponLevel", "LEVEL top-left"),
    ("weaponLevel", "LEVEL bottom-right"),
    ("weaponRank", "RANK top-left"),
    ("weaponRank", "RANK bottom-right"),
]


class WeaponRoiRecorder:
    def __init__(self) -> None:
        self.root = tk.Tk()
        self.root.title("Weapon ROI recorder")
        self.root.attributes("-topmost", True)
        self.root.geometry("380x200+40+40")
        self.root.resizable(False, False)

        self.status = tk.StringVar(value="Weapon tab open? Press Start.")
        self.step = tk.StringVar(value="Idle")
        self.last = tk.StringVar(value="")

        tk.Label(self.root, textvariable=self.status, wraplength=360, justify="left").pack(
            anchor="w", padx=10, pady=(10, 4)
        )
        tk.Label(self.root, textvariable=self.step, font=("Segoe UI", 11, "bold")).pack(
            anchor="w", padx=10, pady=4
        )
        tk.Label(self.root, textvariable=self.last, fg="#444", wraplength=360).pack(
    ...
    def start(self) -> None:
        ...
        self.status.set("Click corners in game. Window stays on top.")
    ...
    def _run(self) -> None:
        ...
        self.root.after(0, lambda: self.status.set("All corners captured."))
    ...
    def _save(self) -> None:
        out = ROOT / "debug_out" / f"weapon_rois_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        (ROOT / "debug_out" / "_latest_weapon_rois.txt").write_text(str(out), encoding="utf-8")
